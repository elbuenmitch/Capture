...
sliderZoom.oninput = function(e) {
  ...
  //dispImgLeft = Math.max(Math.min(dispImgLeft*(1-lastZoomChange), cropperLeft+scaleOffsetX), -origImgW+cropperLeft+cropperWidth-scaleOffsetX);
  //dispImgTop = Math.max(Math.min(dispImgTop*(1-lastZoomChange), cropperTop+scaleOffsetY), -origImgH+cropperTop+cropperHeight-scaleOffsetY);
...
function dragElement(elmnt) {
    ...
    function elementDrag(e) {
      ...
      scaleOffsetX = ((origImgW*(dispImgZoom-1))/2);
      scaleOffsetY = ((origImgH*(dispImgZoom-1))/2);
      //dispImgOriginX = Math.min(Math.max(dispImgOriginX + pos1, cropperWidth/2+((origImgW*(dispImgZoom-1))/2)), -(cropperWidth/2)+origImgW);
      dispImgLeft = Math.max(Math.min(dispImgLeft - pos1, cropperLeft+scaleOffsetX), -origImgW+cropperLeft+cropperWidth-scaleOffsetX);

      //Arranges the vertical axis
      //dispImgOriginY = Math.min(Math.max(dispImgOriginY + pos2, cropperHeight/2), -(cropperHeight/2)+origImgH);
      dispImgTop = Math.max(Math.min(dispImgTop - pos2, cropperTop+scaleOffsetY), -origImgH+cropperTop+cropperHeight-scaleOffsetY);
